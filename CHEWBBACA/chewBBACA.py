...
def run_predict_cdss():
	...
	parser = argparse.ArgumentParser(prog='PredictCDSs',
									 description='Predict CDSs for a set of genomes.',
									 usage=msg(),
									 formatter_class=ModifiedHelpFormatter,
									 epilog='Module documentation available at https://chewbbaca.readthedocs.io/en/latest/user/modules/PredictCDSs.html')
	...
	parser.add_argument('-i', '--input-files', type=str,
						...
						help='Path to the directory that contains the input '
							 'FASTA files, in FASTA or GenBank format')
	...
	parser.add_argument('--t', '--translation-table', type=pv.translation_table_type,
						...
						help='Genetic code used to predict CDSs. This value '
							 'is ignored if a valid training file is passed '
							 'to `--ptf`, `--training-file`.')
	...
	parser.add_argument('--tr', '--training-reference', type=str,
						...
						help='Path to the reference genome used to create '
							 'a training file to predict CDSs.')
...
@pdt.process_timer
def run_get_alleles():
	"""Run the GetAlleles module to get the alleles identified during allele calling."""
	...
	parser = argparse.ArgumentParser(prog='GetAlleles',
									 description='Get the alleles identified during allele calling.',
									 usage=msg(),
									 formatter_class=ModifiedHelpFormatter,
									 epilog='Module documentation available at '
...
def run_compute_distances():
	"""Run the ComputeDistances module to compute pairwise distances."""

	def msg(name=None):
		usage_msg = 'chewBBACA.py ComputeDistances --input-file <file> --output-file <file> [options]'

		return usage_msg

	parser = argparse.ArgumentParser(prog='ComputeDistances',
									 description='Compute pairwise distances.',
									 usage=msg(),
									 formatter_class=ModifiedHelpFormatter,
									 epilog='Module documentation available at '
	...
	parser.add_argument('--m', '--method', type=str, required=False,
						...
						dest='method',
						help='Distance calculation method. The module supports '
	...
	parser.add_argument('--outfmt', '--output-format', type=str, required=False,
						...
						dest='output_format',
						help='Output format for the output file.')

	parser.add_argument('--no-mask', action='store_true',
						required=False, dest='no_mask',
						help='Do not mask the allelic profiles.')

	parser.add_argument('--similarity', action='store_true',
						required=False, dest='similarity',
						help='Compute similarity instead of distance.')

	parser.add_argument('--cpu', '--cpu-cores', type=pv.verify_cpu_usage,
						required=False, default=1, dest='cpu_cores',
						help='Number of CPU cores/threads that will be '
							 'used to run the process (chewie resets to '
							 'a lower value if it is equal to or exceeds '
							 'the total number of available CPU cores/threads).')
...
def main():
	...
	functions_info = {'PredictCDSs': ['Predict CDSs from inpput genomes.',
									  run_predict_cdss],
					  ...
					  'HashProfiles': ['Hash allelic profiles.',
									   run_hash_profiles],
					  'GetAlleles': ['Get the alleles identified in a dataset and save them to FASTA files',
					  				 run_get_alleles],
